...
Import replacement-comparison projection coverage proves corrected pre-cutover arithmetic where raw physical history differs, correction/reversal/restoration semantics, strict cutover/later exclusion, Adjustment and ExcelImport inclusion exactly once, persisted previous-ImportRun count/net separation, one `PositionAsOf(CutoverDate - 1 day)` call, controlled projection-failure propagation with no persistence/raw fallback, schema-16 no-projection compatibility and the `DateOnly.MinValue` empty-domain case. `ExecuteAsync` operational reconciliation remains deliberately unchanged and pending.

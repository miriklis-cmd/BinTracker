...
While intentional failures existed, normal full-suite and `Build-BinTracker.bat` runs were temporarily suspended. That was not a waiver. BT-20-P1 closes the final nine ordinary reds, so full suites and the canonical BAT are required again. No permanent filter or skip is allowed, and red evidence is never passing coverage or operator acceptance. Task19 Print Pack implementation remains frozen; R7 changes remain limited to the four demonstrated result-affecting gaps.

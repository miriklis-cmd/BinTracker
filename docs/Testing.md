...
Normal composition still supplies the no-op initial-lineage writer and remains schema 16. Explicit isolated schema-17 composition alone exercises the SQLite writer; tests must continue proving that the default writer performs no schema probe/query/write. Core construction validation remains provider-neutral, Services remains the client-neutral Single/Batch authority, and Data owns SQLite mechanics. No PostgreSQL/API/web/mobile/handheld execution evidence is implied.

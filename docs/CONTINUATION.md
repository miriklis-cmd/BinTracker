...
**Purpose:** Preserve the uncommitted task 19 Daily Print Pack shared-snapshot implementation, its evidence and pending review/user-run canonical gate, alongside the frozen lineage semantics and later separately authorized activation sequence.

**Reconciled:** 11 September 2026 (Australia/Sydney)

**Final Task-19 review update:** The user reports no remaining production-code blocker after final independent review. The final documentation-only correction clarifies the established BT-ARCH-004 internal EF boundary; it does not change production/test source or reopen the shared-snapshot implementation. This supersedes pending independent-review references below for the corrected production code. Jack's canonical BAT remains pending; this pass does not authorize running it, starting Task 20, activation, staging, committing or pushing. TEST REQUIRED: None for the wording correction; existing Task-19 operator/real-print gates remain separate.
...
## Active task 19 — shared Print Pack snapshot prerequisite
...
- All task changes remain uncommitted/unstaged; HEAD/upstream remain at the starting checkpoint. Unrelated `.codex-evidence/` (12 evidence/archive files), `BIN-LIN-IMP-03A-HANDOFF.tmp`, `BinTracker-MarketFloor-Checkpoint-R4.ps1` and `github claude.ps1` were not modified. No new untracked source or artifact was introduced.
- Next safe action: independent ChatGPT source/diff review, then Jack runs `Build-BinTracker.bat`. TEST REQUIRED: Targeted, including real preview/print after separately authorized activation. Source/automated/build proof does not imply full-suite/canonical, retained-database, Windows/operator, print, package or PostgreSQL evidence. Do not begin runtime activation or later work in this task.
...
- **Current task 19 starting/final HEAD:** `8b389588aec7e7b2c4ff702fac901bb8dca3408c`, synchronized with `origin/codex/movement-correction` at 0 ahead / 0 behind. Task 19 edits are uncommitted/unstaged; mechanically verify them rather than assuming a clean worktree.
...
At the historical fbcf20f checkpoint, Daily Print Pack required no production change. `DailyPrintPackService` deliberately remains composed through the existing projection-capable `IOutstandingReportService` and `IDailyMovementsReportService`; adding direct projection calculations there was rejected as redundant architecture drift. Task 19 now uses that dependency solely for transaction coordination, retaining both reports as the only section-policy owners. Explicit isolated schema-17 composition proves transitive `PositionAsOf` plus exact-date `Activity`, with correction, reversal, consumed-original and superseded-generation semantics still owned by those underlying consumers. Relevant projection failure propagates without raw/schema-16 fallback. After independent review, the canonical BAT passed source/package-state audit and all 605/605 tests with 0 failed, 0 skipped, 0 warnings and 0 errors. This remains source/build/automated evidence: real preview/print operator acceptance is pending. That historical checkpoint did not prove a cross-section snapshot. Task 19 now shares a provider-owned read transaction across both delegated sections and customer/container metadata, with deterministic committed-writer tests; independent review and Jack's canonical BAT remain pending.
...
-> `Daily Print Pack shared cross-section snapshot prerequisite (task 19 implemented/tested; independent review and Jack's BAT pending)`
...
The stages through unified mutation execution, root CAS/idempotency/provider translation, corrected activity/PositionAsOf authority, dormant read-side consumer preparation, SaveSingle transaction participation and Import comparison/execution reconciliation are implemented at their recorded dormant source/automated levels. The projection authority remains unregistered and unactivated. The next safe action is independent review and Jack's canonical BAT for task 19's dormant shared-snapshot change; atomic schema-17/runtime cutover follows only with separate authorization. Do not activate schema 17, register runtime lineage/projection, rehearse a retained database or start UI/post-v1 work without separate authorization.
...
1. Independent review and Jack's canonical BAT for task 19's implemented dormant Daily Print Pack shared-snapshot prerequisite.
2. Atomic schema-17/runtime cutover (separate authorization required).
...
Task 19 does not authorize runtime activation. Its shared-snapshot source/concurrency evidence requires independent review and Jack's canonical BAT; broader failure/concurrency/full gates and retained-database/Windows acceptance remain separate.

Task 18 independent review and the user-run canonical BAT are complete. Active task 19 resolves only the Daily Print Pack snapshot prerequisite, followed by independent ChatGPT review and Jack's canonical BAT. Preserve task 18's single transaction-participating projection authority, distinct replacement/new-import temporal scopes, previous ImportRun deletion/provenance evidence, schema-16 compatibility and fail-closed behavior. Atomic activation, retained-database rehearsal and later UI work require separate authorization.

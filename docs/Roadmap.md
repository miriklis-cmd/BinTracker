...
**Architecture frozen; dormant persistence/current-resolution and IMP-05/05B/05C planning foundations externally reviewed and approved, IMP-06 audit primitive locally implemented, runtime cutover not begun:** BT-CORR-018..033, BT-HIST-008..009, BT-AUD-015..017 and BT-OPS-011..012 define stable roots/lines, full generations, restoration/RemainReversed, corrected activity/PositionAsOf, root concurrency, provider/client neutrality, migration preflight/backup and fail-closed health. Core vocabulary, migration-safety infrastructure, an isolated schema-16→17 migrator/postflight and an unregistered validation-gated CURRENT-root resolver are dormant. The corrected exact-fact trusted planning snapshot and pure complete-generation mutation/physical-output planner add explicit per-Reversed-line whole-root decisions, complete result-pointer semantics, separate authoritative business date and generic import/adjustment exclusion. IMP-05C additionally proves each current terminal reversal is the exact persisted equal/opposite Manual neutraliser of LastEffective with no import/batch membership and rejects future current-pair/effective dates. The resolver/materializer validate only the selected current snapshot under one read boundary; they are not a report/writer/UI cutover or full-history diagnostic. IMP-06 adds only an unregistered persistence-boundary appender for one primary AuditEvent in a caller-owned DbContext/transaction; it owns no save or commit and the existing independent AuditService is unchanged. Schema 17 remains dormant and unregistered; production schema-16 authority, activation and runtime cutover remain unchanged. IMP-07 remains unstarted and unauthorized. The retained Batch #30 partial reversal is protected acceptance evidence. Alpha.8 physical-batch-only eligibility remains the current safe runtime implementation until migration activation and the complete service/report/UI change are delivered coherently.

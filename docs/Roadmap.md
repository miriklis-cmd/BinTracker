...
**Architecture frozen; dormant entry and unified-mutation execution canonically verified, runtime cutover not begun:** BT-CORR-018..033, BT-HIST-008..009, BT-AUD-015..017 and BT-OPS-011..012 define stable roots/lines, full generations, restoration/RemainReversed, corrected activity/PositionAsOf, root concurrency, provider/client neutrality, migration preflight/backup and fail-closed health. Core vocabulary, migration-safety infrastructure, an isolated schema-16→17 migrator/postflight and validation-gated CURRENT-root resolution remain dormant. IMP-05C provides the exact-fact trusted snapshot and complete-generation planner; IMP-06 provides the caller-transaction primary-audit primitive; IMP-07 provides atomic native `Initial` generation-zero lineage. The unified mutation slice now executes Correct/Reverse/Restore under explicit isolated schema-17 composition with target-root audit health, exact persisted generation-line identity, replay/idempotency, root CAS, provider-specific transient-contention translation, atomic primary audit and optional physical output. Canonical `Build-BinTracker.bat` passed 538/538 automated tests with 0 failed/skipped and 0 warnings/errors. Normal composition still injects no-op initial and mutation writers and does not register the SQLite schema-17 writers. Schema 17 remains dormant/unregistered; the production DbContext/startup catalogue and schema-16/alpha.8 runtime authority are unchanged. The retained Batch #30 partial reversal remains protected acceptance evidence. Alpha.8 physical-batch-only eligibility remains the current safe runtime implementation until migration activation and the complete projection/service/UI change are delivered coherently.
...
Unified Correct/Reverse/Restore execution and its root-CAS/idempotency/provider-translation stage are now implemented and canonically verified at the dormant source/automated level. The next dependency-ordered lineage work is corrected activity/PositionAsOf and atomic projection/cutover, which remains unstarted and requires separate authorization. Runtime schema activation, retained-database rehearsal, Restore UI and Windows/operator acceptance are not silently implied.

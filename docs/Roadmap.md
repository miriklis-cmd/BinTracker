...
Task20C (12 September 2026) freezes the user-approved R1–R4 activation decisions in Architecture and adds ordinary unskipped characterization/future-behavior tests only. That20C pass contained no production implementation or schema activation. Current scope is typed Single response evidence, logical preview/command closure, minimum native audit/review safety and the one Data startup funnel, with R5–R7 validation/scope constraints. Full Audit/History detail remains the next separate milestone after cutover; Restore/RemainReversed UI remains separately sequenced afterward. Reversed-line whole-root workflows stay blocked until explicit choices can be collected. Task20D implements the production R4/R5 Data coordinator foundation and isolated A2/A3/A8–A11/A13–A15 lifecycle proof. Task20E implements only R1 Single response receipt/replay semantics under explicit controlled schema17 composition. Task20F implements only R2 there: provider-neutral preview/intent contracts carry stable logical identity/current state and captured generation into the existing writer/CAS, enabled native17 rejects all legacy mutation writes, and whole-root preview plus direct execution require every current line Active regardless of embedded Restore/RemainReversed decisions. Normal schema16 startup remains dormant until atomic cutover closes R3; audit/review closure, EF membership alignment and R7 fixes remain pending. No retained-data/operator acceptance is implied. Normal full-suite/BAT runs are temporarily suspended while the remaining intentional-red tests exist, and those same tests must become green in the eventual canonical gate.
...
Unified Correct/Reverse/Restore execution, root-CAS/idempotency/provider translation, the reusable corrected activity/PositionAsOf authority, dormant read consumers, Import comparison/execution reconciliation and Task 19's Daily Print Pack shared-snapshot prerequisite are implemented at their recorded dormant source/review/canonical-build levels. R3 audit/review safety, EF immutable membership and the four R7 snapshot gaps remain before normal schema-17/runtime activation. The next implementation run defaults to the largest coherent safe Task20 roadmap pass that can close and independently verify genuine acceptance boundaries without loosening frozen semantics; it is not automatically divided into historical micro-slices. Audit/history integration, retained-database rehearsal, Restore UI, Windows/operator acceptance, packaging/deployment and alternative-provider acceptance are not silently implied.

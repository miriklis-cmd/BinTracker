...
Task20C (12 September 2026) freezes the user-approved R1–R4 activation decisions in Architecture and adds ordinary unskipped characterization/future-behavior tests only. That20C pass contained no production implementation or schema activation. Current scope is typed Single response evidence, logical preview/command closure, minimum native audit/review safety and the one Data startup funnel, with R5–R7 validation/scope constraints. Full Audit/History detail remains the next separate milestone after cutover; Restore/RemainReversed UI remains separately sequenced afterward. Reversed-line whole-root workflows stay blocked until explicit choices can be collected. Task20D subsequently implements the production R4/R5 Data coordinator foundation and isolated A2/A3/A8–A11/A13–A15 lifecycle proof. Normal schema16 startup remains dormant until atomic cutover closes R1/R2/R3; the logical preview bridge, receipt, audit/review closure, EF membership alignment and R7 fixes remain pending. No retained-data/operator acceptance is implied. Normal full-suite/BAT runs are temporarily suspended while intentional-red tests exist, and the same tests must become green in the eventual canonical gate.

...
Task20C (12 September 2026) freezes the user-approved R1–R4 activation decisions in Architecture and adds ordinary unskipped characterization/future-behavior tests only. There is no production implementation or schema activation in this pass. Current scope is typed Single response evidence, logical preview/command closure, minimum native audit/review safety and the one Data startup funnel, with R5–R7 validation/scope constraints. Full Audit/History detail remains the next separate milestone after cutover; Restore/RemainReversed UI remains separately sequenced afterward. Reversed-line whole-root workflows stay blocked until explicit choices can be collected. Tests requiring the missing coordinator/preview bridge remain specifically blocked; no test harness substitutes for those production contracts. Normal full-suite/BAT runs are temporarily suspended while intentional-red tests exist, and the same tests must become green in the eventual canonical gate.

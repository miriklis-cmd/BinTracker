...
Task20C froze the user-approved R1–R4 activation decisions. Task20D/E/F/P1/FIX1 implemented the coordinator, R1 receipts, R2 preview/CAS/legacy closure, minimum R3 routing, EF membership and R7 snapshot prerequisites under explicit schema17 composition. BT-20-P2 now activates those seams atomically in normal WinForms composition: the Data coordinator gates host exposure and developer replacement, the catalogue ends at17, native entry/mutation writers and the transactional projection authority are normal registrations, and WinForms correction/reversal routes use logical previews and captured generations. BT-20-P3 completes full native Audit/History detail at source/automated-evidence level; Restore/RemainReversed UI is the next separate milestone. Retained-database rehearsal, Windows/operator acceptance and Task20 global completion remain pending.

...
Unified Correct/Reverse/Restore execution, root-CAS/idempotency/provider translation, the reusable corrected activity/PositionAsOf authority, dormant read consumers and Import replacement comparison are implemented at their recorded dormant source/automated levels. The next dependency-ordered production slice is Import execution operational reconciliation. Runtime schema activation, the Daily Print Pack shared-snapshot prerequisite, audit/history integration, retained-database rehearsal, Restore UI and Windows/operator acceptance are not silently implied.

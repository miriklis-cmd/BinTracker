...
Current planning baseline: **v0.5.0-alpha.9.4**
...
Task20C froze the user-approved R1–R4 activation decisions. Task20D/E/F/P1/FIX1 implemented the coordinator, R1 receipts, R2 preview/CAS/legacy closure, minimum R3 routing, EF membership and R7 snapshot prerequisites under explicit schema17 composition. BT-20-P2 activates those seams atomically in normal WinForms composition, BT-20-P3 completes full native Audit/History detail, and BT-20-P4 implements the separately sequenced Restore/RemainReversed WinForms workflow at source/focused-test level. Retained-database rehearsal, Windows/operator acceptance and Task20 global completion remain pending.

BT-20-FIX2 corrects the startup compatibility defect exposed before P4 UI smoke: schema16 preflight now recognizes the historically migrated V13 no-self-FK form as well as the exact fresh-schema16 form, and schema17 publication finalizes and requires the exact self-FK. This is source and synthetic-test evidence only; the retained database remains unmigrated, Batch #30 remains untouched and P4 Windows smoke must be rerun.

BT-20-FIX3 follows the alpha.9.1 canonical pass and disposable Windows smoke. That smoke reached the P4 dialog and passed interception, no-default, Cancel/Esc no-write and tested-scale visibility, but operator comprehension failed because raw IDs dominated the row. Alpha.9.2 makes authoritative customer/container/direction/quantity/date and reversal context primary while retaining lineage IDs as secondary evidence. The corrected dialog, disposition outcomes and continuation still require Windows retest; retained rehearsal and Batch #30 remain untouched.

BT-20-FIX4 follows the alpha.9.2 canonical pass and expanded disposable Windows smoke. Restore/RemainReversed, native detail, a subsequent whole-root correction, corrected current projection and Correct Selected controls passed. `Correct Entire Batch` from the resulting lineage-output physical batch failed because WinForms compared that batch with immutable original `RootMovementBatchId`; restoration history wording and the dialog's smaller default were bounded presentation defects. Alpha.9.3 uses typed authoritative original/output anchors without changing root identity, distinguishes persisted Restoration from Reversal and increases only the dialog's default size. Corrected Windows acceptance, retained rehearsal and protected Batch #30 remain pending.

Task 19 is complete at pushed commit `930f2c77b1df7cd4629be2faff662750431854fe`; independent review and its canonical BAT are complete. BT-20-P2 now composes that Daily Print Pack shared-snapshot authority in normal schema17 runtime. Windows/operator real preview/print, retained-database rehearsal, packaging/deployment and alternative-provider acceptance remain pending.
...
**Architecture frozen; normal schema17/native runtime activated at source and automated-evidence level:** BT-CORR-018..033, BT-HIST-008..009, BT-AUD-015..017 and BT-OPS-011..012 define stable roots/lines, full generations, restoration/RemainReversed, corrected activity/PositionAsOf, root concurrency, provider/client neutrality, migration preflight/backup and fail-closed health. BT-20-P2 routes normal startup through the Data coordinator, BT-20-P3 adds native Audit/History detail, and BT-20-P4 adds the explicit separately sequenced Restore/RemainReversed workflow. The production DbContext still does not map lineage entities; Data-owned native SQL remains the provider adapter. Explicit schema16 compatibility fixtures retain their legacy authority without becoming a second normal runtime. Retained Batch #30 rehearsal and Windows/operator acceptance remain pending.
...
Unified Correct/Reverse/Restore execution, root-CAS/idempotency/provider translation, corrected activity/PositionAsOf, all prepared consumers, Import reconciliation, Task 19's shared Print Pack snapshot, native Audit/History detail and the separately sequenced Restore/RemainReversed UI are now normal post-readiness behavior. The frozen subsequent sequence remains: broader activated-system failure/concurrency/full automated gates; retained-database migration/backup/recovery rehearsal; Windows/operator acceptance including protected Batch #30 and real report/detail/review behavior; the whole-codebase layer-delineation audit; then Security Hardening and subsequent roadmap work. Packaging/deployment and alternative-provider acceptance are not silently implied.

...
**Architecture frozen; dormant foundations through IMP-07 independently reviewed/approved at their recorded evidence levels, runtime cutover not begun:** BT-CORR-018..033, BT-HIST-008..009, BT-AUD-015..017 and BT-OPS-011..012 define stable roots/lines, full generations, restoration/RemainReversed, corrected activity/PositionAsOf, root concurrency, provider/client neutrality, migration preflight/backup and fail-closed health. Core vocabulary, migration-safety infrastructure, an isolated schema-16→17 migrator/postflight and an unregistered validation-gated CURRENT-root resolver remain dormant. IMP-05C provides the exact-fact trusted snapshot and pure complete-generation planner; IMP-06 provides the caller-transaction primary-audit primitive. IMP-07 now implements atomic native `Initial` generation-zero lineage for newly successful Single/Batch entry only through explicit isolated schema-17 composition, preserving the existing service as business authority, first-successful Batch request order, exact retry behavior, existing entry audit actions and all-or-nothing failure. Normal composition still injects a no-op writer that performs no schema access. Schema 17 remains dormant/unregistered; the production DbContext/startup catalogue and schema-16/alpha.8 runtime authority are unchanged. The retained Batch #30 partial reversal remains protected acceptance evidence. Alpha.8 physical-batch-only eligibility remains the current safe runtime implementation until migration activation and the complete mutation/projection/service/UI change are delivered coherently.
...
The next dependency-ordered lineage work is the later unified mutation execution sequence: Correct / Reverse / Restore, followed by the frozen root-CAS/idempotency/provider-translation and corrected-projection/cutover stages. Generation-zero redesign, runtime schema activation, retained-database rehearsal and Windows/operator acceptance are not silently folded into that next slice.

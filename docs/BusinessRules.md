...
Task20F implements that R2 boundary only under explicit native17 composition. A preview resolves the stable root and permanent logical lines from persisted identity, exposes the validated current state and captures `ExpectedGeneration`; execution uses that value unchanged. The execution boundary applies the Active-only whole-root policy to the already-materialized trusted snapshot before planning, without another query or state calculator. The three alpha.8 correction/reversal write routes fail with a typed controlled outcome before writing when the native writer is enabled. Dormant schema16 composition keeps the accepted alpha.8 behavior. This adds no second planner, current-state calculation or lineage authority.

BT-20-P1 implements the remaining pre-activation safety boundaries without activating schema17. Native correction/reversal detail and review use the complete persisted audit-to-operation request/generation/root shape and validate affected-root audit health; any partial or contradictory native shape fails closed instead of downgrading to alpha.8, while a migrated legacy operation with no native envelope remains eligible for the legacy path. Native identifiers never enter alpha.8 physical-ID or payload reconstruction, and unsupported native detail returns no unrelated evidence. A loaded immutable movement/batch membership is not nulled by EF before the schema17 database rejects deletion. When one Import comparison, Dashboard, customer-search or customer-summary result combines result-affecting evidence/metadata and operational position, both are read through the existing provider-consistent snapshot authority. Schema16 behavior remains compatible.

Single Entry, Batch Entry, reversal and import treat `ClientOperationId` as command identity. Exact canonical replay returns the original persisted result where truthful persisted result evidence exists; reusing the ID with a different payload is rejected. The sole approved Task20 R1 exception is a migrated/pre-receipt Single: recognize identical command identity, create no duplicate, rewrite no lineage and return controlled legacy-replay-result-unavailable instead of fabricating missing historical response numbers. Batch and logical mutation exact-result replay guarantees are unchanged. Task20E implements this for explicitly composed native17 Single commands; normal schema16 Single retains its dormant compatibility calculation. Database uniqueness remains authoritative under races. Container Type names use normalized `NameKey`, and only one current ImportRun owns a cutover date.

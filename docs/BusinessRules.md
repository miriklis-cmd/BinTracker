...
Task20 R1–R4 are frozen verbatim in `docs/Architecture.md` (12 September 2026). For activated-schema17 Single Entry, the immutable typed command-response receipt records movement/command identity, captured authoritative business date and the signed resulting position from the validated post-lineage caller transaction. BOTH the receipt and audit record that same authoritative projection position; the audit does not obtain it from the receipt. Any failure rolls back the complete attempt. The receipt is command-response evidence only, never current-balance, projection, lineage-state, report or audit-source authority. Audit prose/JSON is never balance or result authority. Exact retry returns this receipt despite later activity. An identical migrated/pre-receipt command creates no duplicate or lineage rewrite but returns a specific controlled legacy replay result unavailable outcome when the original response cannot truthfully be recovered; this is the approved compatibility exception, not permission to invent a receipt or parse audit prose.

Activated correction/reversal uses logical mutation authority and preview-captured generation only. All-Active whole-root date/direction correction remains available; any Reversed line blocks direct correction. Supplying decisions inside the blocked Correct+WholeRoot command does not make it executable. The UI separately requires Restore or Remain Reversed for every current reversed line, sends all Restore choices through one existing atomic Restore mutation, leaves retained lines unchanged, re-previews authoritative state and continues correction only when the fresh root is all-Active. Cancellation and all-RemainReversed create no artifact. Review validates affected native operation/audit health through authoritative entity/operation identity.

Normal activated schema17 composition uses the Task20F R2 boundary. A preview resolves the stable root and permanent logical lines from persisted identity, exposes the validated current state and captures `ExpectedGeneration`; execution uses that value unchanged. The execution boundary applies the Active-only whole-root policy to the already-materialized trusted snapshot before planning, without another query or state calculator. The three alpha.8 correction/reversal write routes fail with a typed controlled outcome before writing. Explicit schema16 compatibility composition keeps the accepted alpha.8 behavior. This adds no second planner, current-state calculation or lineage authority.

A physical batch selected for whole-root correction is an anchor, not logical identity. The original `RootMovementBatchId` and a lineage-native `LogicalMovementPhysicalOutput` may each resolve to the same stable root only through authoritative persisted relationships. Zero, multiple, malformed or cross-root associations fail closed; the selected output batch never becomes the root identity and execution still uses the fresh current generation and current effective lines.

The BT-20-P1/FIX1 safety boundaries are inherited by normal activated runtime. Native correction/reversal detail and review use the complete persisted audit-to-operation request/generation/root shape and validate affected-root audit health; any partial or contradictory native shape fails closed instead of downgrading to alpha.8, while a migrated legacy operation with no native envelope remains eligible for the legacy path. Native detail projects that validated operation's root, generations, permanent lines, state/action/masks, source and transformation ledger evidence; native identifiers never enter alpha.8 physical-ID or payload reconstruction. A loaded immutable movement/batch membership is not nulled by EF before the schema17 database rejects deletion. Import comparison, Dashboard, customer search and customer summary combine result-affecting evidence/metadata and operational position through the existing provider-consistent snapshot authority. Schema16 compatibility behavior remains available only to explicit fixtures/tools that deliberately omit activated authorities.

Single Entry, Batch Entry, reversal and import treat `ClientOperationId` as command identity. Exact canonical replay returns the original persisted result where truthful persisted result evidence exists; reusing the ID with a different payload is rejected. The sole approved Task20 R1 exception is a migrated/pre-receipt Single: recognize identical command identity, create no duplicate, rewrite no lineage and return controlled legacy-replay-result-unavailable instead of fabricating missing historical response numbers. Batch and logical mutation exact-result replay guarantees are unchanged. Normal schema17 Single uses the Task20E typed receipt; explicit schema16 compatibility Single retains the legacy calculation. Database uniqueness remains authoritative under races. Container Type names use normalized `NameKey`, and only one current ImportRun owns a cutover date.
...
- Under normal activated schema17 composition, execution calculates that baseline in its caller-owned serializable transaction. Replacement uses corrected truth strictly before cutover and excludes the selected previous ImportRun once; ordinary new import retains its all-representable-dates whole-ledger baseline. Projection failure or cancellation cannot fall back to raw history or leave an Import write. Explicit schema16 compatibility composition retains its legacy calculation.
...
- Daily Movements reports authoritative operational activity by `MovementDate`. Normal schema17 runtime uses validated current-generation projection so correction/restoration bookkeeping cannot inflate totals; explicit schema16 compatibility retains alpha.8 effective-movement suppression.
...
- Explicit schema16 operational views suppress correction-consumed originals/neutralisers and show the corrected replacement. Normal schema17 runtime preserves that accepted result through validated current-generation projection. Movement History/Audit always retain complete evidence.
...
- A current Reversed line is trusted for mutation only when its TerminalReversal persistently reverses that exact LastEffective movement, has the opposite direction and identical customer/container/quantity, uses Manual provenance with no ImportRun or physical-batch membership, and neither current movement date is after the injected authoritative business date. An Active current effective movement must likewise not be future-dated. Current-pointer and OrdinaryReversal role proof remains mandatory but cannot override contradictory immutable movement facts.
...
- Whole-root correction requires the authoritative request to name every permanent line and carry an explicit Restore or RemainReversed decision for every and only already-Reversed line. Active lines use the root correction selection; each restored line uses its own restoration override selection. RemainReversed retains both current pointers, zero contribution and no fake movement. AlreadyMatches and CarriedForward write state evidence but no ledger row.
...
- AppliedFieldMask records the exact line-level explicit override selection that governed the requested result, including selected-but-equal fields. Corrected/AlreadyMatches require the complete nonempty correction selection; Restored records the exact override selection and may be None; Initial/MigrationBaseline/CarriedForward/Reversed/RemainReversed are None. Mask alone never proves change, no-op, movement creation, contribution or physical output.

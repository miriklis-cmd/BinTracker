...
Activated correction/reversal uses logical mutation authority and preview-captured generation only. All-Active whole-root date/direction correction remains available; any Reversed line blocks the current workflow until the later UI collects explicit Restore/RemainReversed choices. Review must validate affected native operation/audit health. Native audit routing uses authoritative entity/operation identity and may return controlled unsupported detail pending the later UI milestone.

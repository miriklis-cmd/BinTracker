...
Activated correction/reversal uses logical mutation authority and preview-captured generation only. All-Active whole-root date/direction correction remains available; any Reversed line blocks both preview and execution until Restore/RemainReversed is separately sequenced. Supplying those decisions inside the blocked Correct+WholeRoot command does not make it executable. Review must validate affected native operation/audit health. Native audit routing uses authoritative entity/operation identity and may return controlled unsupported detail pending the later UI milestone.

...
Normal schema17 runtime uses the frozen R2 client-neutral preview bridge. It resolves selected persisted movement/batch anchors or a stable logical root to permanent logical line IDs, validated current business/state data and the captured expected generation. Logical execution uses that exact generation for root-wide CAS; it does not refresh to latest. The three legacy alpha.8 mutation commands fail closed before artifacts, while explicit schema16 compatibility composition retains accepted legacy behavior. Whole-root preview and execution are eligible only when all current lines are Active. The execution boundary rejects a Reversed line from the already-materialized trusted snapshot even if the caller supplies valid Restore/RemainReversed decisions; no line is omitted or restored, and those decisions remain separately sequenced behind the later UI.

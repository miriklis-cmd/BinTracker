...
Normal schema17 runtime uses the frozen R2 client-neutral preview bridge. It resolves selected persisted movement/batch anchors or a stable logical root to permanent logical line IDs, validated current business/state data and the captured expected generation. For a physical batch, the preview separately reports authoritative `RootOriginal` or `LineageOutput` anchor evidence; `RootMovementBatchId` remains the sole original-batch identity. Logical execution uses the captured generation unchanged for root-wide CAS; it does not refresh to latest. The three legacy alpha.8 mutation commands fail closed before artifacts, while explicit schema16 compatibility composition retains accepted legacy behavior. Whole-root correction is eligible only when all current lines are Active. The execution boundary rejects a Reversed line from the already-materialized trusted snapshot even if the caller supplies valid Restore/RemainReversed decisions. BT-20-P4 collects those decisions separately, restores selected lines through one existing Restore command, refreshes the preview and re-enters correction only from fresh eligible state.

Normal activated runtime inherits the BT-20-P1/FIX1 blockers closure. Audit detail/review classifies the complete persisted operation request/generation/root shape: partial or contradictory native evidence fails closed, genuine migrated alpha.8 evidence has no native envelope/generation association and remains on the legacy path, and healthy native evidence uses affected-root validation. Native detail presents the validated logical root, operation/generation identity, permanent line state/action/mask, original/prior/result/terminal physical evidence and transformation provenance without parsing alpha.8 payloads or treating a logical root as a physical movement. EF preserves tracked immutable movement-batch membership and Data validates schema16 `SET NULL` versus schema17 `RESTRICT` as distinct catalogue shapes. Import replacement comparison, Dashboard, customer search and customer summary obtain every result-affecting metadata/evidence read and operational projection from one provider-consistent snapshot.

Task20 activation follows the verbatim approved R1–R4 in `docs/Architecture.md`: Single first success publishes physical evidence, Initial lineage, validated PositionAsOf(captured business today), typed response receipt and audit in one transaction, then commits/returns. Retry replays the original receipt; identical pre-receipt legacy retry has the explicit controlled unavailable-result exception. Validated operational projection supplies the authoritative committed-snapshot position to BOTH receipt and audit; neither receipt nor audit prose/JSON is balance/projection/lineage authority, and receipt is not audit-source authority. BT-20-P2 makes this the normal post-readiness schema17 path, BT-20-P3 supplies full native Audit/History detail and BT-20-P4 supplies the separately sequenced Restore/RemainReversed UI. Current position always uses validated operational projection. Correction/reversal/restoration requires stable logical root/line intent and preview-captured expected generation; activated legacy writes fail closed.
...
- BT-MOVE-014: Explicit schema16 whole persisted Batch Entry correction changes common date/direction for every physical line and never partially succeeds. Normal schema17 runtime supersedes physical-batch-only eligibility with complete logical-root planning while preserving that compatibility guard.
...
- BT-CORR-018..033: Native lineage correction resolves a stable logical root/line, previews the complete expected generation, supports Restore/RemainReversed/AlreadyMatches, applies only explicit field changes, rejects complete no-op and commits one full root generation atomically. Restore/RemainReversed presentation is implemented-static; retained-data and operator acceptance remain separately gated.
- The reviewed mutation planner preserves absent/explicit-clear/value intent and gives each planned line the exact AppliedFieldMask required by its action. A whole-root correction supplies an explicit Restore or RemainReversed decision for every already-Reversed line, with per-line restoration overrides; active lines retain correction semantics. Selected-but-equal correction fields remain present on AlreadyMatches; override-free Restore remains substantive with mask None. The trusted snapshot cross-proves each current terminal reversal against the exact persisted LastEffective relationship and equal/opposite ordinary-reversal business facts, excluding non-Manual/import/batch-member terminals. The authoritative business date is separate trusted planner input, not client intent, and rejects future current effective or terminal rows. The complete plan carries existing or typed plan-local result pointer semantics for persistence. Normal native mutation consumes this pure, write-free planner; the current WinForms milestone uses its existing separate Restore operation and never bypasses the Correct+WholeRoot block.
- The operator sees current/result values and understandable Changed / Already correct / Previously reversed—remain / Restore states. Reversed-line disposition leads with authoritative customer, container, direction, quantity, date and plain-language reversal context; logical root/line/original/last/reversal/generation IDs remain available as secondary technical evidence. The client sends stable IDs, expected generation, explicit intent and reason; services supply display values from authoritative data and recompute authoritative output.

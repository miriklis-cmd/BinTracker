...
Task20D established the R4/R5 coordinator, Task20E R1 receipt/replay, Task20F R2 preview/CAS/legacy closure, and BT-20-P1/FIX1 the bounded R3, EF-membership and R7 safety prerequisites. BT-20-P2 changes their normal composition and startup authority. BT-20-P3 completes native Audit/History detail by projecting the already validated operation/root/generation/line/ledger evidence through a provider-neutral service result. Before it attributes historical prior or transformation evidence, Data proves the predecessor has this root/permanent line and the operation's exact preceding generation, and proves the ledger owner and introducing generation-line have the same permanent line. BT-20-P4 adds the client-neutral explicit disposition bridge and WinForms dialog: all Restore choices form one separately sequenced existing Restore mutation, RemainReversed creates no restoration, and correction is re-entered only from a fresh all-Active preview. BT-20-FIX3 keeps human-readable customer/container labels in that provider-neutral preview contract, loaded with the validated snapshot under the service-owned read transaction; WinForms renders them as primary operational information while retaining lineage IDs as secondary evidence and never reconstructs identity or queries provider tables. BT-20-FIX4 makes a resolved physical-batch anchor explicit as either the immutable root-original batch or authoritative lineage-output evidence. The anchor never replaces `RootMovementBatchId`; WinForms consumes the provider-neutral proof and does not query SQLite or duplicate root resolution. Retained-database rehearsal, corrected Windows/operator acceptance, the protected layer audit and Security Hardening remain separate.
...
Schema16 baseline capability inspection is catalogue-aware rather than a projection of future schema17 alone. Historical V13 upgrades legitimately have the `ReversesMovementId` column and unique partial index without a persisted self-FK; current `EnsureCreated` schema16 has the exact `ReversesMovementId -> BinMovements.Id` RESTRICT relationship. Data accepts only those two forms, rejects malformed or ambiguous alternatives, and the governed schema17 table rebuild adds the exact relationship when upgrading the historical form. Native schema17 validation always requires exactly one authoritative self-FK.
...
The unified schema17 mutation path composes that planner with atomic Correct/Reverse/Restore persistence, root-wide expected-generation CAS, canonical idempotency/replay, optional truthful physical output and one operation-linked primary audit. Provider-owned lookups resolve physical selection anchors to the stable root; Services maps the trusted snapshot to immutable preview DTOs; execution consumes the preview generation without refresh. Correct+WholeRoot always rejects a Reversed current line. BT-20-P4 first requires an explicit Restore/RemainReversed choice for every reversed preview line, submits all Restore choices as one separate existing Restore command, refreshes the root, and only then permits the unchanged all-Active correction workflow. Normal native17 composition rejects `ReverseAsync`, `CorrectAsync` and `CorrectBatchAsync` before alpha.8 writes; explicit schema16 compatibility composition retains them. Persisted generation-line identity comes only from `LogicalMovementGenerationLines.Id`; provider conflicts are classified after rollback/fresh inspection. Normal composition registers `SqliteMovementMutationWriter`.

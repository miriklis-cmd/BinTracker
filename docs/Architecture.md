...
Task20D implements the R4/R5 foundation as `SqliteStartupDatabaseCoordinator` in Data, available only through explicit isolated composition. Task20E implements only R1 Single-entry response receipts and replay under the same explicit schema17 test composition. Task20F implements R2 there: client-neutral previews resolve a selected physical movement/associated batch or stable logical root to permanent logical line IDs, validated current state and the captured expected generation; execution retains that generation for the existing root-wide CAS, while enabled native17 composition rejects every alpha.8 mutation write route. Whole-root preview and execution are eligible only when every logical line is Active; an explicit Restore or RemainReversed decision cannot bypass that activation policy and must instead be separately sequenced. Normal WinForms `Program`, `DatabaseSetup`/schema16 catalogue, dormant writers and consumer registrations are unchanged. The coordinator can return a validated schema17-ready session for a future activated host; it does not install that host or expose schema17 writers to old composition. Atomic normal-startup/developer-marker integration remains gated on R3 closure; R7 and later acceptance remain outstanding.

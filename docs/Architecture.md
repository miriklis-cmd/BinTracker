...
Task20D implements the R4/R5 foundation as `SqliteStartupDatabaseCoordinator` in Data, available only through explicit isolated composition. Normal WinForms `Program`, `DatabaseSetup`/schema16 catalogue, dormant writers and consumer registrations are unchanged. The coordinator can return a validated schema17-ready session for a future activated host; it does not install that host or expose schema17 writers to old composition. Atomic normal-startup/developer-marker integration remains gated on R1/R2/R3 closure; R7 and later acceptance remain outstanding.

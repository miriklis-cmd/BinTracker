...
Task20D implements the R4/R5 foundation as `SqliteStartupDatabaseCoordinator` in Data, available only through explicit isolated composition. Task20E implements R1 Single-entry response receipts/replay and Task20F implements R2 there: client-neutral previews preserve stable logical identity/current state and the captured generation through the existing writer/CAS, enabled native17 composition rejects every alpha.8 mutation write route, and whole-root preview/execution require every line Active. BT-20-P1 closes the remaining pre-activation implementation blockers under that same explicit composition: complete persisted native operation/request/generation/root shape controls audit detail/review with affected-root health validation; a partial or contradictory native shape cannot downgrade to legacy, while a migrated alpha.8 operation with no native generation envelope remains supported. EF tracking preserves immutable batch membership while schema16 and schema17 retain their distinct persisted delete actions; and the four characterized Import/Dashboard/customer result boundaries use the existing provider-consistent snapshot seam. Normal WinForms `Program`, schema16 migration catalogue, dormant writers and consumer registrations remain unchanged. The coordinator can return a validated schema17-ready session for a future activated host; it does not install that host or expose schema17 writers to old composition. Atomic normal-startup/developer-marker integration remains the next separate boundary; full native Audit/History detail, Restore/RemainReversed UI, broader activated-system gates, retained-database rehearsal and operator acceptance remain sequenced afterward.
...
- Single Entry, Batch Entry, reversal and import persist client operation IDs. Exact canonical replay returns the original persisted result where truthful persisted result evidence exists; reuse with a different payload is rejected. Under explicit schema17 composition, Task20E implements R1 with an immutable typed Single receipt and its sole compatibility exception: a migrated/pre-receipt Single recognizes identical command identity without duplication or lineage rewrite, but returns the controlled legacy-replay-result-unavailable outcome rather than fabricated prior numbers. Batch and logical mutation exact-result replay guarantees remain intact. Normal schema16 Single retains its dormant compatibility behavior and may recompute a retry balance; it is not the activated17 authority.
...
IMP-07 adds an explicit isolated initial-lineage writer behind the existing Single/Batch `MovementService`, which remains the sole authority for authentication, authorization, validation, master-data checks, idempotency, physical movement/batch creation, audit and returned results. Normal composition supplies dormant no-op lineage and Single-receipt stores that perform no schema probe, query or write; only isolated schema-17 composition supplies their SQLite implementations and transaction-participating projection. For a newly committed eligible Single/Batch entry, the lineage writer creates one generation-zero `Initial` logical root atomically, with one permanent line per original movement, zero for the Single ordinal and first-successful request enumeration order for Batch ordinals. It completes RootOriginal ownership and introduction links, validates the temporary provider-neutral graph, then activates the root at generation 0. Task20E extends only the Single path: after that lineage publication, the existing projection authority produces PositionAsOf the once-captured business date in the caller transaction; one typed response receipt and the existing audit persist the same signed result before commit. Exact native retry reads the receipt without re-projecting, while a migrated/pre-receipt retry returns the controlled typed unavailable outcome. It creates no generation-zero correction operation or physical-output link and does not change existing entry audit actions. Equivalent reordered Batch retries preserve the committed root and ordinals, and migrated `MigrationBaseline` roots remain unchanged. SQLite transaction/schema mechanics stay in Data; Core owns the typed immutable response concept and provider-neutral construction invariants, while Services owns the client-neutral workflow. This source and targeted evidence do not activate schema 17, register lineage in the production DbContext/startup catalogue, or implement any PostgreSQL/API/client work.
...
The unified schema-17 mutation slice composes that planner with atomic Correct/Reverse/Restore persistence, root-wide expected-generation CAS, canonical operation idempotency/replay, optional truthful physical output and one operation-linked primary audit. Task20F adds only its client-neutral preview/activation boundary: provider-owned lookups resolve physical selection anchors to the existing stable root, Services maps the already trusted planning snapshot to immutable preview DTOs, and `ExecuteLogicalAsync` consumes the preview-captured generation without refresh. After generation validation and before the general planner, that service boundary rejects Correct+WholeRoot whenever the same trusted snapshot contains a Reversed line, regardless of supplied Restore/RemainReversed decisions; those decisions remain valid lower-planner semantics for the separately sequenced workflow. Enabled native17 composition also rejects `ReverseAsync`, `CorrectAsync` and `CorrectBatchAsync` before alpha.8 writes; dormant schema16 composition retains them. Persisted generation-line identity comes only from `LogicalMovementGenerationLines.Id`; no logical-line, movement, ordinal or other identity is substituted. SQLite constraint/CAS conflicts are reclassified only after rollback and fresh committed-state inspection, while unresolved SQLite busy/locked exhaustion follows the stable persistence-failure path and cannot become stale-generation or lineage-integrity diagnosis. The primary audit records trusted current lineage and business state before the change and the resulting generation, per-line action/state/mask/pointers/business state and relevant new/output movement identities after it. This execution remains available only through explicit isolated schema-17 composition; normal composition registers `DormantMovementMutationWriter`, not `SqliteMovementMutationWriter`. Normal startup activation, projection/report cutover and Restore UI are not implemented by this slice.
...
The following R1–R4 wording is frozen exactly as approved. These are activation requirements; the Task20D R4/R5 foundation and Task20E R1 isolated implementation/evidence are described above, without claiming complete activation or acceptance. Task20C supplied ordinary unskipped characterization/future-behavior tests. Normal startup remains dormant/schema16. R1 explicitly refines prior broad Single retry wording for pre-receipt commands; no historical result is fabricated.

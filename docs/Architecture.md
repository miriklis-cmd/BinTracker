...
Import replacement comparison now follows the same optional dormant-consumer pattern. When an explicit projection authority is supplied, `CompareReplacementAsync` obtains the corrected position through the day before cutover from one `PositionAsOf` query, then removes the previous ImportRun's persisted pre-cutover effect by authoritative ImportRun ownership before reconciliation. The previous run's comparison count and net effect remain physical forensic evidence. Projection failure propagates without raw fallback; `DateOnly.MinValue` has an empty pre-cutover domain without date subtraction. With no authority, the schema-16 raw comparison is unchanged. This does not change Import `ExecuteAsync`, register the projection authority or activate schema 17.

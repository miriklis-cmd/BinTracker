...
| BT-AUD-007 | v1 | IMPLEMENTED-STATIC | Audit Trail visibly exposes `Needs review`, `Reviewed` and not-applicable/blank state and All/Needs review/Reviewed filtering. Login/reminder routing selects and scrolls to the deterministic oldest pending event. Windows/manual acceptance pending. | USER-REQUEST,CURRENT-DOC,CODE |
| BT-AUD-008 | v1 | IMPLEMENTED-STATIC | `Mark Selected Reviewed` uses deterministic action/required/success/unreviewed eligibility; contextual confirmation precedes the exact-event update, immediate state/count/next-item feedback follows it, reviewer/time persist, acknowledgement is audited with event identity, and duplicates fail closed. Windows/manual acceptance pending. | USER-REQUEST,CURRENT-DOC,CODE |
| BT-AUD-009 | v1 | IMPLEMENTED-STATIC | Detail action is disabled for no selection/unrelated events and enabled only for authoritative MovementBatch or implemented movement-change lineage detail. Ordinary batch detail remains keyed by persisted MovementBatch ID. Windows/manual acceptance pending. | USER-REQUEST,CURRENT-DOC,CODE |
| BT-AUD-010 | candidate | IMPLEMENTED-STATIC | Correction/reversal audit events route to exact persisted lineage detail and MovementBatch events to persisted batch detail; missing/duplicate identity fails closed. ImportRun routing remains a future release decision. Windows/manual acceptance pending. | USER-REQUEST,CURRENT-DOC,CODE |
...
| BT-AUD-013 | v1 | IMPLEMENTED-STATIC | Administrator sessions display a persistent non-blocking navigation-wide WinForms review panel with live count and pending-set action; it refreshes after review and disappears at zero. The login popup remains. Presentation consumes service state/change contracts; future WinUI 3 replaces only the panel with native InfoBar. Windows/manual acceptance pending. | USER-REQUEST,CURRENT-DOC,CODE |
| BT-AUD-014 | v1 | IMPLEMENTED-STATIC | Esc closes Movement Change Detail or Batch Detail back to its owning Audit Trail; Esc in Audit Trail closes it back to the underlying BinTracker screen and never terminates the application. Windows/manual acceptance pending. | USER-REQUEST,CURRENT-DOC,CODE |

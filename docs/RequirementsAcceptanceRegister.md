...
| BT-AUD-007 | v1 | PLANNED-V1 | Audit Trail visibly exposes `Needs review`, `Reviewed` and not-applicable/blank review state and provides practical All/Needs review/Reviewed filtering so an Administrator need not infer outstanding review work from action, user or description text. Requirement recorded; implementation and Windows/manual acceptance pending. | USER-REQUEST,CURRENT-DOC |
| BT-AUD-008 | v1 | PLANNED-V1 | `Mark Selected Reviewed` is enabled only for a selected, unreviewed, review-required Operator movement correction/reversal. It remains disabled for no selection, Administrator changes, unrelated event classes, already-reviewed events and every other non-reviewable event. Successful acknowledgement persists reviewed status, reviewer and UTC review time, writes its own audit event, and cannot be repeated. Requirement recorded; implementation and Windows/manual acceptance pending. | USER-REQUEST,CURRENT-DOC |
| BT-AUD-009 | v1 | PLANNED-V1 | `View Batch Detail` is disabled for no selection and non-batch events and enabled only when the selected audit event has authoritative persisted MovementBatch detail; existing persisted-line detail remains authoritative. Requirement recorded; implementation and Windows/manual acceptance pending. | USER-REQUEST,CURRENT-DOC |
| BT-AUD-010 | candidate | NEEDS-CONFIRMATION | Future contextual Audit Trail drill-down should route supported event types to authoritative existing detail surfaces: MovementBatch to persisted batch detail, ImportRun/Excel Import with an authoritative run ID to Import Run History/detail, and correction/reversal to authoritative lineage detail when implemented; events without meaningful detail expose no enabled detail action. No additional routes are currently claimed implemented. | USER-REQUEST,CURRENT-DOC |
| BT-AUD-011 | v1 | PLANNED-V1 | End-to-end Windows/regression acceptance must prove Operator correction/reversal -> persisted review requirement -> later Administrator login and consolidated notification -> visibly discoverable Needs review item -> eligible acknowledgement -> Reviewed state with persisted reviewer/time -> acknowledgement audit event -> duplicate acknowledgement prevented. | USER-REQUEST,CURRENT-DOC |
...
| BT-AUD-013 | v1 | PLANNED-V1 | Administrator sessions display a persistent, non-blocking review infobar across main navigation while Operator movement correction/reversal reviews remain outstanding. It shows the live count and clear review message, opens the pending Audit Trail set, refreshes after review changes and disappears at zero. It is Administrator-only and never an approval gate; the existing login popup remains. Review count/state/navigation use presentation-independent contracts: current WinForms may use a reusable infobar-style control/panel and future WinUI 3 replaces only presentation with native InfoBar. Requirement recorded; implementation and Windows/manual acceptance pending. | USER-REQUEST,CURRENT-DOC |
...
| BT-CORR-009 | v1 | IMPLEMENTED-STATIC | Whole-batch correction uses persisted MovementBatch identity only, supports common date/direction change, and is all-or-nothing if any line is ineligible. | CHAT-SURFACED,CODE |
...
| BT-CORR-015 | v1 | IMPLEMENTED-STATIC | Effective operational views suppress correction-consumed originals and correction-only neutralisers, then apply date/customer/container/direction/source filters and totals to the corrected replacement; immutable Movement History/Audit retain and clearly label all lineage, while ordinary reversal visibility is unchanged. | USER-REQUEST,CURRENT-DOC,CODE,TEST |

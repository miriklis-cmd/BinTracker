...
Current baseline: **v0.5.0-alpha.9.4**
...
## Historical lineage prerequisite evidence (superseded by BT-20-P2 composition)

Task 19 completion reconciliation: implementation and independent review are complete at pushed commit `930f2c77b1df7cd4629be2faff662750431854fe`. Jack's canonical `Build-BinTracker.bat` passed at v0.5.0-alpha.8.7 with mechanical audit (259 permanent IDs, 27 Markdown files), restore, build and 632/632 automated tests (0 failed, 0 skipped). This remains dormant preparation evidence only; schema 17 is unregistered/inactive and runtime activation, Windows/operator real preview/print, retained-database rehearsal, packaging/deployment and alternative-provider acceptance remain unproven. No requirement status is promoted by this reconciliation.
...
BIN-LIN-IMP-07 supplies reviewed static/targeted evidence for BT-CORR-031 through an explicit isolated schema-17 initial writer behind the existing provider-neutral Single/Batch service. Normal production composition remains schema 16 with dormant no-op initial and mutation writers, so that source implementation does not imply startup activation, retained-database rehearsal or operator acceptance. The broader BT-CORR-018..030/032..033 and BT-AUD lineage requirements retain their existing status until their complete scopes are delivered.
...
The Import comparison/execution reconciliation slices add focused static/automated evidence toward BT-IMP-008/010/011/013/022 and the lineage projection requirements: explicit dormant composition uses corrected pre-cutover operational truth for replacement, excludes the prior ImportRun exactly once while preserving physical count/net evidence, preserves the ordinary-import whole-ledger scope through `DateOnly.MaxValue`, includes Adjustment/ExcelImport once, participates in the caller-owned serializable execution transaction, fails closed on projection failure/cancellation without raw fallback or surviving writes, and preserves schema-16 behavior. Runtime schema activation and operator acceptance remain pending, so no requirement status is promoted by these dormant slices.

Task BT-CODEX-20260910-19 adds dormant source/review/canonical-build evidence toward the existing BT-CORR-029 snapshot invariant and BT-PACK-001..003 behavior: one Print Pack's PositionAsOf, exact-date activity and result-affecting customer/container metadata share one provider-consistent transaction. Deterministically interleaved committed correction/reversal/restoration and metadata updates cannot mix sections; relevant projection failure/cancellation yields no PDF or success audit. This clarifies application of an existing requirement, adds no ID or scope, and promotes no status. Runtime activation, retained-database and Windows/real-print acceptance remain separate; canonical BAT evidence is complete with 632/632 tests passed, 0 failed and 0 skipped.

Task 19 review clarification — 11 September 2026: the user explicitly reaffirmed the client-neutral/provider-neutral layer rule. BT-ARCH-004's factory/context is an internal Services/Data boundary, never a client-facing report API. BT-ARCH-008's PostgreSQL direction is a likely future provider rather than an exclusive architectural dependency; another suitable SQL provider must preserve all business semantics. The two rows are clarified below with stable IDs, unchanged scopes/statuses and no new provider/client implementation. This corrects the exposed public report participation surface under the existing architecture requirements; it does not authorize a wider persistence refactor.

## Task20 activation decision record — 12 September 2026

User-approved R1–R4 are frozen verbatim in Architecture; R5–R7 refine their validation and scope. No new permanent ID is needed: R1 extends BT-CORR-026/029/031 and builds on implemented command identity under BT-ARCH-013; R2 extends BT-ARCH-016 and BT-CORR-022/024/027; R3 extends BT-AUD-009/010/015/016 and BT-HIST-008; R4/R5/R6 extend BT-ARCH-017, BT-CORR-030/032 and BT-OPS-011/012. R7 applies existing BT-CORR-029/033 snapshot evidence. Task20E/F/P1/FIX1 established the prerequisite static boundaries; BT-20-P2 supplies normal composition while complete native History/detail, retained database and operator criteria remain pending.

BT-20-P2 activates that reviewed composition in normal WinForms runtime without adding, removing, renumbering or changing the scope of a permanent requirement. `Program -> DatabaseSetup.InitializeAsync -> IStartupDatabaseCoordinator` withholds application scopes until schema17 readiness and retains the returned session through host disposal. Normal DI supplies native initial-lineage, Single receipt, mutation and transactional projection authorities; normal correction/reversal uses logical previews and captured generations; staged developer Load/Fresh is claimed/revalidated by the same coordinator. BT-20-P3 supplies native Audit/History detail from the same validated operation/root/generation authority. BT-20-P4 supplies the separately sequenced Restore/RemainReversed WinForms workflow. Broad lineage/audit/history/operations rows remain `PLANNED-V1` because retained-database rehearsal, Windows/operator evidence and Task20 global acceptance remain pending.

BT-20-FIX2 adds no requirement ID or status promotion. It corrects enforcement of existing BT-CORR-030/032 and BT-OPS-011/012: Data accepts the historically valid schema16 V13 no-self-FK form or the exact current fresh-schema16 self-FK, rejects malformed alternatives before backup or mutation, and schema17 publication/native validation finalizes and requires the exact RESTRICT self-FK. Synthetic coordinator evidence is not retained-database rehearsal or Windows acceptance.

R1 explicitly refines broad Single replay wording: new activated17 entries persist the exact successful typed response atomically with physical movement, Initial lineage, validated in-transaction PositionAsOf(captured business today) and matching audit. Validated operational projection supplies the authoritative committed-snapshot position to BOTH receipt and audit. Receipt is command-response evidence only, never balance/projection/lineage/report/audit-source authority; audit prose/JSON is neither balance nor result authority. Identical pre-receipt legacy retry cannot duplicate or rewrite lineage and returns controlled legacy replay result unavailable if the exact original result cannot be recovered. No invented backfill, result JSON framework or audit-text authority is allowed. This approved compatibility exception does not weaken changed-payload conflict or Batch/logical mutation exact-result replay.

R2 requires preview-captured root generation with no latest-generation substitution, closure of active17 legacy writes, preserved all-Active whole-root date/direction intent and blocking direct whole-root correction whenever a Reversed line remains. BT-20-P4 sequences explicit Restore/RemainReversed choices separately and re-previews before correction. Normal runtime uses the reviewed Task20F resolver/materializer/planner/writer/CAS boundary; explicit schema16 composition remains compatible. BT-20-P1 supplies R3 routing/affected-root health, EF membership and four R7 snapshots. R4's one Data-owned funnel covers creation, pre16 progression, governed16→17 activation, native17 startup, future-schema rejection and developer replacement/reclassification. R5 validates native capability/current health; R6 limits companion-lease claims to participating processes. Operator acceptance remains pending.

Task20 frozen R2 acceptance trace (reconciled by `BT-20F-FIX2`):

| Acceptance rule | Authoritative source | Enforcement path | Proof/evidence | Current status |
|---|---|---|---|---|
| Active17 legacy correction/reversal writes fail closed | Architecture Decision R2 | `MovementCorrectionService.RejectLegacyMutationIfNativeEnabled` at `ReverseAsync`, `CorrectAsync`, `CorrectBatchAsync` entry | `Enabled_schema17_rejects_legacy_commands_without_appending_alpha8_artifacts` plus normal-composition activation proof | IMPLEMENTED-STATIC in normal native17 composition; operator acceptance pending |
| Preview resolves stable logical identity/current state | Architecture Decision R2; BT-ARCH-016/BT-CORR-018..020 | `PreviewLogical*Async` -> existing `IMovementMutationWriter` selection lookup -> existing ready/materialize authority -> immutable preview DTO | `Native_correction_uses_the_preview_identity_and_generation_and_replays_exactly` | Static R2 evidence; broader BT-ARCH-016/BT-CORR-018..020 remain PLANNED-V1 |
| Preview captures ExpectedGeneration and execution does not replace it | Architecture Decision R2; BT-CORR-027 | preview maps trusted root `CurrentGenerationNumber`; command persists the supplied generation through canonical intent and `TryPublishAsync` CAS | native correction/reversal stale-preview tests plus normal-composition correction/reversal | IMPLEMENTED-STATIC under BT-CORR-027; operator acceptance pending |
| Stale generation loses without write artifacts | Architecture Decision R2; BT-CORR-027 | trusted snapshot generation comparison plus conditional root CAS and fresh-state classification | `Captured_generation_loses_after_another_command_advances_the_root`; stale-preview assertions in the preview test | IMPLEMENTED-STATIC under BT-CORR-027 |
| Direct Correct+WholeRoot rejects any trusted current Reversed line | Architecture Decision R2 | `ExecuteLogicalAsync`, after trusted materialization/generation validation and before the sole planner call, throws `WholeRootCorrectionUnavailable` | `Native_whole_root_correction_rejects_reversed_lines_even_with_explicit_decisions` (Restore and RemainReversed) with unchanged generation/movement/generation/operation/audit/output/legacy-line state | IMPLEMENTED-STATIC at explicit execution boundary; operator acceptance pending |
| Restore/RemainReversed cannot be folded into the blocked whole-root workflow but remain lower-planner semantics | Architecture Decision R2, read with the frozen planner semantics in BT-CORR-022 | service activation guard blocks direct correction; `ResolveReversedLinesAsync` separately validates the complete reversed-line decision set and delegates Restore choices to the existing mutation path | mixed planner/bypass controls plus `Task20ReversedDispositionWorkflowTests` | P4 IMPLEMENTED-STATIC presentation/workflow boundary; broader BT-CORR-022 remains PLANNED-V1 |
| Schema16 compatibility remains deliberate | Architecture Decision R2 | explicit compatibility fixtures retain `DormantMovementMutationWriter`, legacy routes and a catalogue ending16; normal application DI is native17 | `Dormant_schema16_keeps_legacy_correction_and_does_not_offer_logical_preview`; schema16 correction/reversal and migration fixture regressions | Compatibility remains supported without becoming normal authority |

Task20 P1 acceptance trace:

| Acceptance rule | Authoritative source | Enforcement path | Proof/evidence | Current status |
|---|---|---|---|---|
| Native detail/review uses persisted association and never substitutes logical-root IDs or alpha.8 payload text | Architecture Decision R3; BT-AUD-009/010/015/016; BT-HIST-008 | `AuditService` transaction -> `IMovementMutationWriter.InspectAuditAssociationAsync` -> complete persisted AuditEvent/operation request/version/root/expected/result/generation association shape -> existing affected-root audit-health validator; any partial native indicator fails closed, while a root-associated migrated operation with no native envelope remains legacy; healthy native detail returns controlled no-detail | seven `Task20AuditActivationTests`, including independent detail/review mixed-shape corruption and genuine migrated-alpha.8 detail/review, plus audit characterization | P1/FIX1 IMPLEMENTED-STATIC boundary; full native detail UI and broad BT-HIST-008 remain PLANNED-V1 |
| Loaded immutable membership is not nulled by EF and catalogue delete actions remain exact | BT-CORR-032; Database schema16/schema17 contract | EF `ClientNoAction`; Data-owned fresh-schema16 compatibility DDL; catalogue-aware capability validation requires schema16 `SET NULL` and schema17 `RESTRICT` | membership activation/characterization plus complete startup/migration regression suites | Static mismatch closure active normally; broad BT-CORR-032 remains PLANNED-V1 pending rehearsal |
| Import replacement evidence/projection, Dashboard attention/projection, customer-search metadata/projection and customer-summary metadata/projection each share one snapshot | BT-CORR-029/033; Architecture R7 | existing `ReadSnapshotAsync`/`QueryInTransactionAsync` seam; no UI rule or raw fallback | four `Task20SnapshotActivationTests`, endpoint scenarios, normal composition and full suites | Static four-gap closure active normally; broad BT-CORR-029/033 remain PLANNED-V1 pending acceptance |
| R1/R2/schema16 compatibility boundaries are preserved through activation | Architecture Decisions R1/R2 and Task20 scope | unchanged R1 receipt/R2 paths; normal DI selects native writers and projection; explicit compatibility composition selects dormant writers and catalogue16 | Task20 activation suites, BT-20-P2 normal-composition tests, full Unit/Integration suites and source audit | Preserved in activated normal runtime; operator acceptance pending |

Task20 P3 acceptance trace:

| Acceptance rule | Authoritative source | Enforcement path | Proof/evidence | Current status |
|---|---|---|---|---|
| Native detail exposes validated operation/root/generation/line and physical evidence without a legacy fallback | BT-AUD-009/010/015/016; BT-HIST-008; Architecture R3 | `AuditService` transaction -> classifier/affected-root health -> `ReadNativeAuditEvidenceAsync`, which proves prior root/line/exact preceding generation and ledger owner/introduction permanent-line equality -> provider-neutral detail DTO -> existing detail form; native review reads the same evidence before mutation | expected-red cross-line predecessor/introduction 2/2, then focused `Task20Audit` 12/12: correction, reversal, repeated generation, collision, mixed/native corruption, cross-line review rollback and migrated legacy controls | IMPLEMENTED-STATIC for this detail boundary; manual acceptance and broader rows remain pending |

Task20 P4 acceptance trace:

| Acceptance rule | Authoritative source | Enforcement path | Proof/evidence | Current status |
|---|---|---|---|---|
| Every current Reversed line receives an explicit Restore or RemainReversed choice; cancel/incomplete selection writes nothing | Architecture R2; BT-CORR-022 | `ReversedLineDispositionDialog` has no default decision and returns only after all rows are explicit; cancel never calls the service; `ResolveReversedLinesAsync` revalidates complete exact membership | `Viewer_and_incomplete_decisions_leave_no_disposition_artifacts`; Windows cancel/Esc remains manual | IMPLEMENTED-STATIC; operator acceptance pending |
| Multiple Restore choices use one complete existing logical mutation generation; retained lines remain unchanged | BT-CORR-019/021/022/027 | `ResolveReversedLinesAsync` maps the Restore subset to one `MovementMutationRequest.Restore` with preview root/generation; existing planner/writer carries other lines and publishes once | one-line Restore and mixed multiple-line disposition tests; existing all-checkpoint rollback coverage | IMPLEMENTED-STATIC; retained-data/operator acceptance pending |
| All-RemainReversed is a truthful no-write outcome | BT-CORR-022/033 | service validates the current reversed set and returns `NoChange` without calling mutation execution | `Remain_reversed_is_an_explicit_no_write_outcome` exact database-state comparison | IMPLEMENTED-STATIC |
| Stale, unauthorized and retry paths preserve CAS/authorization/idempotency | BT-CORR-026/027; movement authorization rules | service authorizes before reads/no-op, retains the captured expected generation and delegates mutation/replay/conflict classification to `ExecuteLogicalAsync` | stale no-write, Viewer denial, exact replay and changed-operation conflict tests | IMPLEMENTED-STATIC |
| Correction resumes only from a fresh all-Active authoritative preview | Architecture R2; BT-CORR-022/027 | disposition result re-previews by stable root ID; WinForms stops with a controlled message if any line remains Reversed or the refreshed batch/root no longer agrees, otherwise opens the unchanged batch dialog and executes with the fresh generation | workflow tests plus unchanged all-Active whole-root regression; Windows interaction remains manual | IMPLEMENTED-STATIC; operator acceptance pending |
| Reversed-line choices are understandable without resolving database IDs, while forensic identity remains available | Architecture display/client boundary; Functional Specification correction presentation | Services enriches the validated preview with authoritative customer code/name and container name in the same read transaction; WinForms presents customer/container/direction/quantity/date/reversal first and wrapped root/line/original/last/reversal/generation evidence second | `Reversed_preview_carries_authoritative_operator_display_and_forensic_identity`; label-independent stable-ID decision/no-write regression; alpha.9.1 operator feedback; corrected alpha.9.3 Windows retest pending | IMPLEMENTED-STATIC; operator acceptance pending |
| Original and lineage-output physical batches resolve as anchors without changing logical-root identity | BT-CORR-018/019/024/027; Architecture physical-output boundary | Data resolves exactly one persisted root-original or `LogicalMovementPhysicalOutput` relationship and returns a typed provider-neutral anchor; Services pairs it with the validated current snapshot; WinForms validates the selected anchor rather than comparing it with `RootMovementBatchId` | real native-output regression proves original/output anchors, unchanged root batch, same current root, current replacement execution, second correction and missing/ambiguous/cross-root rejection | IMPLEMENTED-STATIC by BT-20-FIX4; Windows retest pending |
| Restoration is distinguishable from ordinary reversal in Movement History without making display text authoritative | BT-CORR-021/023; BT-HIST-008 | Data reads persisted `LogicalMovementLedgerLink.Role` inside the report read transaction; Services derives presentation labels from the provider-neutral transformation role; identity/mutation paths continue to use persisted IDs | native reversal/restoration/correction-neutraliser presentation regression | IMPLEMENTED-STATIC by BT-20-FIX4; Windows retest pending |

The retained Batch #30 requirement remains BT-CORR-033 `PLANNED-V1`. It is protected from mutation/deletion; read-only persisted batch/movement/reversal verification must precede use of approximate display IDs. Its conceptual Blue active `IN 4`, Yellow original `IN 1`, Yellow reversal `OUT 1`, zero-current-Yellow state and complete later operator/Windows matrix are recorded in Database, Testing, TEST-CHECKLIST and the active continuation. No static R2 evidence promotes this acceptance.

## Status / provenance legend

Task20D evidence extends the existing R4/R5/R6 mapping under BT-ARCH-017, BT-CORR-030/032 and BT-OPS-011/012: one production Data coordinator supplies startup classification, governed16-to17 orchestration, native structural/current/projection validation, runtime revalidation and coordinated replacement. Task20E adds R1 Single transaction/receipt/replay proof under BT-CORR-026; Task20F adds R2 preview-captured generation, root-CAS preservation, enabled17 legacy-write closure and the Active-only guard under BT-CORR-027. BT-20-P1/FIX1 close R3, EF-membership and R7 blockers. BT-20-P2 supplies normal runtime and staged-marker composition evidence. BT-CORR-026/027/031 remain `IMPLEMENTED-STATIC`; broader rows remain `PLANNED-V1` pending their complete detail/rehearsal/operator criteria.
...
| BT-ARCH-004 | v1 | IMPLEMENTED-STATIC | Services + `IDbContextFactory<BinTrackerDbContext>` remain the internal application/data-access boundary. Application-facing contracts remain client-neutral and provider-neutral and never expose EF contexts, connections, transactions or provider details. | CHAT-SURFACED,CURRENT-DOC,CODE,USER-REQUEST,DECISION-RECORDED |
...
| BT-ARCH-008 | post-v1 | POST-V1 | Central deployment is desktop/web/phone/other clients through authenticated BinTracker application/API contracts and provider-specific Data to a suitable SQL database. PostgreSQL is a likely future provider, not an exclusive assumption; clients never receive database credentials or connect directly. v1 supplies architectural groundwork only, not the host/provider implementation. | CHAT-SURFACED,CURRENT-DOC,USER-REQUEST,DECISION-RECORDED |
...
| BT-ARCH-013 | v1 | IMPLEMENTED-STATIC | Single Entry, Batch Entry, reversal and import carry persisted client operation identity; identical command reuse recognizes the existing command without duplication, and changed payload reuse is rejected. | CHAT-SURFACED,CURRENT-DOC,CODE |
...
| BT-CORR-026 | v1 | IMPLEMENTED-STATIC | ClientOperationId/fingerprint makes retries exact: same ID/same intent returns the original committed result even after later generations, changed reuse fails, rollback leaves no reservation/artifacts and lost-response retry never duplicates generation/ledger/audit. Normal schema17 Single uses the immutable typed R1 receipt; identical migrated/pre-receipt Single retry lacking truthful historical response evidence has the explicitly approved controlled unavailable-result exception without duplicate or lineage rewrite. Operator acceptance remains pending. | USER-REQUEST,CURRENT-DOC,CODE,TEST |
| BT-CORR-027 | v1 | IMPLEMENTED-STATIC | Normal native17 composition uses root-wide optimistic concurrency to reject stale preview-captured expected generations for correction/reversal/restoration, including different-line races; clients re-preview and v1 provides no per-line merge. Execution never substitutes latest generation, and correctness is independent of SQLite locking. Operator acceptance remains pending. | USER-REQUEST,CURRENT-DOC,CODE,TEST |
...
| BT-CORR-031 | v1 | IMPLEMENTED-STATIC | Under normal activated schema17 composition, each newly successful eligible Manual/Batch entry atomically creates its physical evidence and existing entry audit plus one `Initial` generation-zero root, complete permanent lines/state and RootOriginal/introduction links; failure leaves no new artifact. Batch ordinals preserve first-successful request order, retries never rewrite committed lineage, ImportRun/ExcelImport data remains outside generic lineage, cross-domain references fail closed and import replacement cannot delete referenced evidence. Retained-data and operator acceptance remain pending. | USER-REQUEST,CURRENT-DOC,CODE,TEST |

...
User-approved R1–R4 are frozen verbatim in Architecture; R5–R7 refine their validation and scope. No new permanent ID is needed: R1 extends BT-CORR-026/029/031 and builds on implemented command identity under BT-ARCH-013; R2 extends BT-ARCH-016 and BT-CORR-022/024/027; R3 extends BT-AUD-009/010/015/016 and BT-HIST-008; R4/R5/R6 extend BT-ARCH-017, BT-CORR-030/032 and BT-OPS-011/012. R7 applies existing BT-CORR-029/033 snapshot evidence. BT-ARCH-013 remains confined to implemented command identity, duplicate prevention and changed-payload conflict. Task20E implements the R1 Single receipt/replay boundary, Task20F implements the R2 preview/generation/legacy-write boundary, and BT-20-P1 implements the bounded R3/EF-membership/R7 pre-activation blockers under explicit isolated schema17 composition. The permanent rows remain conservatively unchanged: broader normal runtime, complete native History/detail, retained database and operator criteria are not delivered by this pass.
...
R2 requires preview-captured root generation with no latest-generation substitution, closure of active17 legacy writes, preserved all-Active whole-root date/direction intent and blocking Reversed lines until explicit later UI choices. Task20F implements that boundary under explicit native17 composition by reusing the existing resolver/materializer/planner/writer/CAS; its client-neutral preview supplies stable root/line identity, validated current state and expected generation, all three alpha.8 mutation write routes reject the enabled native writer before artifacts, and the service execution boundary rejects Correct+WholeRoot from that same trusted snapshot whenever any line is Reversed even if Restore/RemainReversed decisions were embedded. Normal schema16 composition remains compatible. BT-20-P1 implements R3's authoritative native routing/affected-root health, the characterized EF membership mismatch, and exactly four R7 snapshot gaps without broadening them. R4 requires one Data-owned funnel for creation, pre16 migration, governed16-to17 activation, native17 startup, future-schema rejection and developer replacement/reclassification. R5 validates structural capabilities and native current health without migration-only prerequisites; R6 limits companion-lease claims to participating processes. No normal runtime or operator acceptance is claimed.
...
| Preview captures ExpectedGeneration and execution does not replace it | Architecture Decision R2; BT-CORR-027 | preview maps trusted root `CurrentGenerationNumber`; command persists the supplied generation through canonical intent and `TryPublishAsync` CAS | `Native_correction_uses_the_preview_identity_and_generation_and_replays_exactly`; `Native_reversal_uses_preview_generation_and_a_stale_preview_never_refreshes_itself` | IMPLEMENTED-STATIC under BT-CORR-027; operator/runtime activation pending |
...
| Schema16 compatibility remains intact/dormant | Architecture Decision R2 | normal DI retains `DormantMovementMutationWriter`; legacy routes remain available only there and normal migration catalogue ends at 16 | `Dormant_schema16_keeps_legacy_correction_and_does_not_offer_logical_preview`; schema16 correction/reversal regression filter | Static compatibility evidence; normal schema17 activation pending |
...
| Loaded immutable membership is not nulled by EF and catalogue delete actions remain exact | BT-CORR-032; Database schema16/schema17 contract | EF `ClientNoAction`; Data-owned fresh-schema16 compatibility DDL; catalogue-aware capability validation requires schema16 `SET NULL` and schema17 `RESTRICT` | membership activation/characterization plus complete startup/migration regression suites | P1 IMPLEMENTED-STATIC mismatch closure; broad BT-CORR-032 remains PLANNED-V1 pending activation/rehearsal |
| Import replacement evidence/projection, Dashboard attention/projection, customer-search metadata/projection and customer-summary metadata/projection each share one snapshot | BT-CORR-029/033; Architecture R7 | existing `ITransactionalOperationalMovementProjectionAuthority.ReadSnapshotAsync` encloses each result's metadata/evidence and `QueryInTransactionAsync`; no UI rule or raw fallback | four `Task20SnapshotActivationTests`, endpoint scenario characterizations and adjacent projection/import/dashboard/balance suites | P1 IMPLEMENTED-STATIC four-gap closure; broad BT-CORR-029/033 remain PLANNED-V1 pending normal activation and acceptance |
| R1/R2/schema16/dormancy boundaries are preserved | Architecture Decisions R1/R2 and Task20 scope | unchanged R1 receipt and R2 service paths; normal DI retains dormant writers; catalogue ends at 16 | complete40-case activation filter, adjacent204-case R1/R2/legacy/mutation/migration/EF/R7 filter, mechanical source inspection | Preserved; normal schema17 runtime remains pending |
...
Task20D evidence extends the existing R4/R5/R6 mapping under BT-ARCH-017, BT-CORR-030/032 and BT-OPS-011/012: one production Data coordinator supplies startup classification, governed16-to17 orchestration, native structural/current/projection validation, runtime revalidation and coordinated replacement through explicit isolated composition. Task20E adds the explicit native17 R1 Single transaction/receipt/replay proof under BT-CORR-026. Task20F adds R2 preview-captured generation, client-neutral stable logical identity/current state, root-CAS preservation, enabled17 legacy-write closure and the Active-only service execution guard under BT-CORR-027. BT-20-P1 closes the four R3, one EF-membership and four R7 intentional-red implementation blockers; BT-20-FIX1 closes the reviewed mixed native/legacy evidence downgrade without changing permanent IDs/scopes/statuses. Normal runtime cutover and legacy staged-marker integration, full native detail, retained-database rehearsal and operator acceptance remain inactive/pending. BT-CORR-026 and BT-CORR-027 remain IMPLEMENTED-STATIC and the broader rows remain PLANNED-V1.
...
| BT-CORR-026 | v1 | IMPLEMENTED-STATIC | ClientOperationId/fingerprint makes retries exact: same ID/same intent returns the original committed result even after later generations, changed reuse fails, rollback leaves no reservation/artifacts and lost-response retry never duplicates generation/ledger/audit. Under explicit dormant schema17 composition, Single response uses the immutable typed R1 receipt; identical migrated/pre-receipt Single retry lacking truthful historical response evidence has the explicitly approved controlled unavailable-result exception without duplicate or lineage rewrite. Normal runtime activation and operator acceptance remain pending. | USER-REQUEST,CURRENT-DOC,CODE,TEST |
| BT-CORR-027 | v1 | IMPLEMENTED-STATIC | Under explicit native17 composition, root-wide optimistic concurrency rejects stale preview-captured expected generations for correction/reversal/restoration, including different-line races; clients re-preview and v1 provides no per-line merge. Execution never substitutes latest generation, and correctness is independent of SQLite locking. Normal runtime activation and operator acceptance remain pending. | USER-REQUEST,CURRENT-DOC,CODE,TEST |
...
| BT-CORR-031 | v1 | IMPLEMENTED-STATIC | Under explicit dormant schema-17 composition, each newly successful eligible Manual/Batch entry atomically creates its physical evidence and existing entry audit plus one `Initial` generation-zero root, complete permanent lines/state and RootOriginal/introduction links; failure leaves no new artifact. Batch ordinals preserve first-successful request order, retries never rewrite committed lineage, ImportRun/ExcelImport data remains outside generic lineage, cross-domain references fail closed and import replacement cannot delete referenced evidence. Normal production composition remains schema 16/no-op until the separately gated activation and rehearsal. | USER-REQUEST,CURRENT-DOC,CODE,TEST |

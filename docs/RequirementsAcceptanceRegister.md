...
BT-20-P2 activates that reviewed composition in normal WinForms runtime without adding, removing, renumbering or changing the scope of a permanent requirement. `Program -> DatabaseSetup.InitializeAsync -> IStartupDatabaseCoordinator` withholds application scopes until schema17 readiness and retains the returned session through host disposal. Normal DI supplies native initial-lineage, Single receipt, mutation and transactional projection authorities; normal correction/reversal uses logical previews and captured generations; staged developer Load/Fresh is claimed/revalidated by the same coordinator. Existing `IMPLEMENTED-STATIC` rows gain normal-composition evidence, but broad lineage/audit/history/operations rows remain `PLANNED-V1` because full native detail, Restore UI, retained-database rehearsal, Windows/operator evidence and Task20 global acceptance remain pending.

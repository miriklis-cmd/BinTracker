...
| BT-AUD-015 | v1 | PLANNED-V1 | Every new movement-change operation atomically creates exactly one primary AuditEvent and review state; review acknowledges the committed operation after it is effective and never acts as preapproval or depends on a physical replacement batch. | USER-REQUEST,CURRENT-DOC |
...
| BT-AUD-017 | v1 | PLANNED-V1 | Legacy correction/reversal audits are associated automatically only by one unique complete structured persisted-ID match; prose, timestamps and matching business values are never authoritative, and unmatched legacy evidence remains independently readable. | USER-REQUEST,CURRENT-DOC |
...
| BT-CORR-020 | v1 | PLANNED-V1 | A current Active line emits exactly one ResultEffectiveMovement; a current Reversed line emits LastEffectiveMovement plus TerminalReversalMovement. Correction neutralisers and superseded episodes remain immutable evidence but not corrected authoritative activity. | USER-REQUEST,CURRENT-DOC |
...
| BT-CORR-022 | v1 | PLANNED-V1 | Whole-root correction supports RemainReversed and partial no-op decisions. Initial, MigrationBaseline, CarriedForward, AlreadyMatches, Corrected, Reversed, Restored and RemainReversed remain distinct actions; complete semantic no-op creates no generation, movement, operation or audit, while restoration alone is substantive. | USER-REQUEST,CURRENT-DOC |
...
| BT-CORR-030 | v1 | PLANNED-V1 | Migration uses only authoritative persisted relationships to create truthful MigrationBaseline state and classifies roots Initializing, Active, ReadOnly or Invalid with stable reasons. Per-database read-only preflight is mandatory; ambiguous lineage is never guessed from business values/timestamps. | USER-REQUEST,CURRENT-DOC |

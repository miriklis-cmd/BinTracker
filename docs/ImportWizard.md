...
The comparison and execution paths are prepared for the dormant lineage runtime. Explicit schema-17 composition obtains the replacement pre-cutover baseline from the single corrected operational projection authority, excludes the selected previous ImportRun exactly once, and executes through the caller-owned serializable transaction. Ordinary new imports use the same authority through `DateOnly.MaxValue`, preserving the accepted whole-ledger temporal scope rather than adopting the replacement cutoff. Projection failure/cancellation is fail-closed with no raw fallback or surviving import write. Previous ImportRun movement count/net remain persisted forensic evidence. Normal schema-16 composition retains existing raw behavior, and schema 17 remains dormant/unregistered.

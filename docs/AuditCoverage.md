...
- The unified mutation writer uses the IMP-06 caller-owned DbContext/transaction appender without changing the independent AuditService. Normal activated composition registers the SQLite writer; explicit schema16 compatibility composition retains the dormant writer.
...
- Production Backup/Restore operations when implemented, plus normal lineage database-upgrade attempts. The coordinator is active, but durable operator-facing upgrade/recovery audit coverage remains pending.
- Lineage preflight, verified pre-upgrade backup/manifest, migration/postflight and recovery attempts require durable evidence. Their active safety enforcement and automated tests do not by themselves constitute complete production audit-log wiring. Recovery evidence must survive restoration to an older database snapshot.

## Evidence expectations

Task20 R3 (verbatim in Architecture) is the minimum activation gate: route by authoritative EntityType and persisted operation association, never interpret a LogicalMovementBatch ID as a BinMovement ID, and never select alpha.8 payload parsing solely from action text. Normal native17 detail/review inspect the complete persisted operation request/generation/root shape in their transaction, validate affected-root audit health before review mutation, and fail closed on any partial or contradictory native indicator. A migrated alpha.8 operation remains legacy only when every native request/generation field is null and no generation references it. Healthy native evidence returns the controlled no-detail result pending the next full native Audit/History milestone. R1's validated projection supplies the committed Single position to both typed receipt and audit; neither receipt nor audit text becomes balance authority.

Task20F changed no audit model or review route and closed only R2. BT-20-P1 adds the minimum R3 routing/review safety without implementing the later native detail UI; BT-20-FIX1 closes the independently reviewed mixed-shape downgrade gap. Enabled native17 detail cannot fall through to legacy parsing, review cannot write until complete native shape and the existing affected-root validator succeed, and unrelated physical evidence cannot be selected by a colliding logical ID.

...
- Task20D implements the R4 Data coordinator/R5 capabilities and real lifecycle proof under isolated composition; Task20E implements R1 Single receipts/replay under explicit schema17 composition. The later atomic cutover must route normal startup and staged developer-marker handling through the coordinator, retain the ready session through host lifetime and close R2/R3 before installing schema17 writers. Existing legacy marker application remains unactivated integration work; the explicit coordinator rejects pending markers without bypass. Companion leases coordinate participating processes only; old/nonparticipating clients must be stopped for later rehearsal/deployment.

...
- [S] Task20D makes the startup cases green, Task20E makes R1 Single green, Task20F makes R2 green, and BT-20-P1 closes the four R3, one EF-membership and four R7 blockers under explicit schema17 composition. BT-20-FIX1 adds mixed-shape native corruption and genuine migrated-alpha.8 proofs. All40 Task20 activation tests are green; canonical `Build-BinTracker.bat` passes audit/restore/build with0 warnings/errors and279/279 UnitTests plus483/483 IntegrationTests,0 failed/skipped. Normal activation/operator acceptance are separate.
- [S] Task20D implements the R4/R5 production coordinator and isolated startup/identity/bootstrap/Load/Fresh lifecycle proof. Normal startup stays schema16/dormant. Retained-database rehearsal and Windows/operator acceptance remain pending.
- [S] Task20C-R1's complete before/after scenarios remain the deterministic R7 proof. BT-20-P1 replaces only the obsolete hybrid-expecting characterization assertions with endpoint checks while preserving the stronger activation assertions, binds R3 to persisted association/affected-root health, and aligns EF tracking with catalogue-specific membership constraints. Startup regressions retain typed A2/A3/A8–A11/A13–A15 and structural coverage. Testing records the green evidence; no normal activation/operator acceptance is claimed.
...
- [S] Replace/Correct comparison and execution preserve legitimate same-day/later activity outside the pre-cutover baseline; explicit dormant schema-17 composition uses corrected projection in the caller-owned serializable execution transaction while retaining physical previous-run evidence until deliberate deletion. Ordinary new import preserves its whole-ledger temporal scope through `DateOnly.MaxValue`; projection failure/cancellation leaves no import write or raw fallback.
...
- [S] IMP-06 adds the persistence-boundary primitive that tracks exactly one new primary AuditEvent in a caller-owned DbContext/active transaction without creating a context/transaction or saving/committing. Focused automated coverage proves caller commit, rollback of both audit and sibling state saved in the same transaction, missing-transaction rejection and unchanged independent `AuditService.WriteAsync`. Dormant schema-17 tests separately prove unique structured association, the RESTRICT operation FK and rejection of a second primary audit for one operation. Independent review approved the bounded implementation/proof; canonical `Build-BinTracker.bat` passed source/package-state audit, restore and Debug build with 0 warnings/errors plus 266 UnitTests and 177 IntegrationTests (443/443 total, 0 failed/skipped). The unified SQLite writer now uses the primitive only under explicit schema-17 composition; normal composition instead registers the dormant mutation writer. Runtime activation and operator acceptance remain pending.
- [S] Unified client-neutral Correct/Reverse/Restore/whole-root command persistence publishes the planned complete generation atomically with exact persisted generation-line identities, root CAS, replay/idempotency, optional physical output and one primary audit. Target-root audit health isolates unrelated roots; exhausted SQLite busy/locked contention translates to PersistenceFailure. Focused tests cover Correct/Reverse/Restore audit payloads and failure paths. Canonical `Build-BinTracker.bat` passed audit/restore/build with 0 warnings/errors and 538/538 automated tests, 0 failed/skipped. Schema activation, retained-database rehearsal, projection/UI cutover and operator acceptance remain pending.
- [S] Task20F adds the minimum client-neutral logical preview bridge under explicit native17 composition. It returns stable root/line IDs, validated current state and preview-captured expected generation; execution preserves that generation for root-wide CAS, stale previews fail without artifacts, all three alpha.8 mutation writes reject before artifacts, and whole-root preview/execution require every line Active even when a caller embeds Restore/RemainReversed decisions. BT-20-P1 closes minimum R3 routing/review safety, EF membership tracking and the four bounded R7 snapshots. Schema16 legacy behavior remains covered. Normal activation and Restore/RemainReversed UI remain pending.
- [S] The dormant provider-neutral corrected activity/PositionAsOf authority projects Active once and Reversed as last-effective plus exact terminal reversal, handles repeated generations/Restore/RemainReversed/ReadOnly, unions Adjustment and ExcelImport once, applies conservative relevance before final filters and fails closed for relevant invalid/incomplete or unexpected unrooted ordinary evidence. Import comparison/execution now explicitly consume it, with execution participating in its caller-owned transaction; other normal operational consumers remain dormant preparations. Focused projection tests passed 10/10 and the earlier canonical gate passed 553/553 with 0 warnings/errors and 0 failed/skipped. Normal schema-16 composition remains unchanged.

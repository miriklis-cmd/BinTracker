...
- [P] Task20D makes the eight startup future cases green through the production coordinator. Task20E makes all eleven current R1 Single cases green, and Task20F makes R2 green, under explicit schema17 composition. Nine ordinary unskipped R3/EF-membership/R7 cases remain intentionally red at their scoped reasons. Full-suite/BAT runs remain temporarily suspended, not waived; all must become green in the later canonical gate.
...
- [P] Task20C-R1 strengthens full before/after R7 comparisons and truthful pre-receipt schema16 command/audit setup. R1 now binds legacy replay to the typed unavailable outcome and proves receipt/projection/audit rollback with real persisted state. R2 activation checks now bind to typed service outcomes; remaining temporary R3 diagnostics must bind to their eventual typed contracts. Startup regressions use typed coordinator failures with A2/A3/A8–A11/A13–A15 and structural rejection coverage. R3/R7 and EF membership remain incomplete before final Task20 acceptance. Targeted green/expected-red evidence is recorded in Testing; no normal activation/operator acceptance is claimed.

...
- [S] BT-20-FIX2 reproduces the historical V13-upgraded schema16 shape without a `ReversesMovementId` self-FK, accepts only that absent form or the exact fresh-schema16 RESTRICT self-FK, fails malformed alternatives closed, and makes the governed schema17 rebuild publish and revalidate the exact final self-FK. The retained database has not been migrated or rehearsed.
- [S] BT-20-FIX3 enriches the client-neutral reversed-line preview with authoritative customer/container display values and makes Customer, Container, Direction, Quantity, Date, reversal context and Decision primary. Technical lineage IDs remain secondary evidence; no decision, disposition or persistence authority moved into WinForms.
- [S] BT-20-FIX4 resolves original and lineage-native output physical batches through one typed authoritative anchor, keeps `RootMovementBatchId` immutable, uses current effective lines for subsequent whole-root correction, rejects missing/ambiguous/cross-root associations, distinguishes Restoration from Reversal through persisted transformation role and increases only the disposition dialog's default client size.
...
- [S] Normal native Correct/Reverse/Restore persistence publishes the planned complete generation atomically with exact generation-line identities, root CAS, replay/idempotency, optional physical output and one primary audit. Target-root audit health and transient-contention failure classification remain covered. Retained-database rehearsal and operator acceptance remain pending.
- [R] Whole-root correction still requires every current line Active. The alpha.9.2 disposable smoke passed understandable disposition, all-RemainReversed, Restore, fresh re-preview, continuation, native detail, a later whole-root correction, current projection and Correct Selected controls, then exposed output-batch whole-root routing plus restoration-label/default-size defects. Retest alpha.9.4 from both original and lineage-output batch anchors, confirm Restoration wording and the `1160x660` default with retained `1050x550` minimum, and rerun mixed/multiple lines, stale messaging and continuation at 1920x1080/150% plus the larger display.

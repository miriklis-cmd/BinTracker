...
Task20C characterization confirms activation blockers in unchanged production code: normal initialization accepts schema17/future schemas without activation health checks; Single uses raw sums and recomputes retry balance after later activity; enabled schema17 composition leaves legacy correction/reversal commands callable; native audit root IDs can collide with unrelated physical movement IDs; review bypasses native operation/audit health; and EF SetNull can nullify tracked immutable membership even though schema17 constraints roll back attempted persisted batch deletion. These defects are not accepted behavior. R1–R4 are frozen in Architecture and covered by targeted unskipped future-behavior tests; no production fix is claimed.

R7 deterministic interleavings prove separate snapshot gaps in Dashboard attention threshold, customer active/name plus position, container activity/name/order plus visible position, and Import comparison's retained previous-run effects versus current projection. Daily Print Pack's shared snapshot remains frozen and is not reopened. Task20C results and blocked coordinator/preview proof items are recorded in Testing/CONTINUATION; full-suite/BAT runs are temporarily suspended while red tests remain.

...
Task20C characterized activation blockers: normal initialization accepts schema17/future schemas without activation health checks; Single uses raw sums and recomputes retry balance after later activity; enabled schema17 composition leaves legacy correction/reversal commands callable; native audit root IDs can collide with unrelated physical movement IDs; review bypasses native operation/audit health; and EF SetNull can nullify tracked immutable membership even though schema17 constraints roll back attempted persisted batch deletion. These defects are not accepted behavior. Task20D fixes startup rejection/readiness through the new production R4/R5 coordinator in explicit isolated composition; all eight startup future cases now pass there. Normal initialization remains unchanged and must join that coordinator only at atomic runtime cutover, after R1/R2/R3 closure. The other20 retained future cases remain intentionally red, including EF membership and R7.

R7 deterministic interleavings prove separate snapshot gaps in Dashboard attention threshold, customer active/name plus position, container activity/name/order plus visible position, and Import comparison's retained previous-run effects versus current projection. Daily Print Pack's shared snapshot remains frozen and is not reopened. Task20D coordinator evidence and remaining receipt/preview/audit/EF/R7 blockers are recorded in Testing/CONTINUATION; full-suite/BAT runs are temporarily suspended while red tests remain.

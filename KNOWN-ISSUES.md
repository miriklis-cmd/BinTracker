...
Task20C characterized activation blockers: normal initialization accepts schema17/future schemas without activation health checks; Single used raw sums and recomputed retry balance after later activity; enabled schema17 composition leaves legacy correction/reversal commands callable; native audit root IDs can collide with unrelated physical movement IDs; review bypasses native operation/audit health; and EF SetNull can nullify tracked immutable membership even though schema17 constraints roll back attempted persisted batch deletion. These defects are not accepted behavior. Task20D fixes startup rejection/readiness through the production R4/R5 coordinator in explicit isolated composition. Task20E fixes the R1 Single behavior under explicit schema17 composition with typed receipts, exact replay and atomic rollback; normal schema16 compatibility still uses the legacy calculation. Normal initialization remains unchanged and must join the coordinator only at atomic runtime cutover, after R2/R3 closure. The remaining13 retained future cases stay intentionally red for R2/R3/EF membership/R7.

R7 deterministic interleavings prove separate snapshot gaps in Dashboard attention threshold, customer active/name plus position, container activity/name/order plus visible position, and Import comparison's retained previous-run effects versus current projection. Daily Print Pack's shared snapshot remains frozen and is not reopened. Task20D coordinator and Task20E receipt evidence plus the remaining preview/audit/EF/R7 blockers are recorded in Testing/CONTINUATION; full-suite/BAT runs are temporarily suspended while red tests remain.

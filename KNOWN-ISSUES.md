...
Task20C characterized activation blockers: startup coordination, R1 Single response/replay, R2 logical mutation routing, R3 native audit/review safety, EF immutable membership and four R7 snapshot gaps. Task20D, Task20E and Task20F implement the first three groups under explicit schema17 composition. BT-20-P1 closes the remaining nine implementation blockers; BT-20-FIX1 additionally closes the reviewed mixed-shape R3 downgrade: complete persisted request/generation/root shape prevents alpha.8/native identity confusion and unhealthy review writes while legitimate migrated legacy evidence remains readable. EF no longer nulls loaded immutable membership, and Import replacement, Dashboard, customer search and customer summary each share one provider-consistent result snapshot. All ordinary Task20 activation tests are green. Normal initialization remains unchanged and must join the coordinator only at the separately authorized atomic runtime cutover; this static blocker closure is not retained-database or operator acceptance.

R7 deterministic interleavings now prove coherent snapshots across Dashboard attention threshold, customer active/name plus position, container activity/name/order plus visible position, and Import comparison's retained previous-run effects versus current projection. Daily Print Pack's shared snapshot remains frozen and was not reopened. Testing/CONTINUATION record the source and full-suite evidence. The prior BAT suspension ended when BT-20-P1 made every ordinary intentional-red test green; the corrected canonical gate passes all762 automated tests with0 failed/skipped and0 build warnings/errors.

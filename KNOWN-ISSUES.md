...
Task 19's reviewed shared-snapshot prerequisite is now part of normal schema17 composition through BT-20-P2. Real preview/print, retained-database rehearsal, packaging/deployment and alternative-provider acceptance remain separate and unproven.

## Whole-batch correction after partial lineage

BT-20-P2 closes the normal-composition activation gap characterized by Task20C: normal initialization now uses the Data coordinator, catalogue17, native entry/receipt/mutation writers and validated projection consumers as one readiness boundary. Developer Load/Fresh markers join the same coordinator and the ready session is held through host lifetime. The prior R1/R2/R3/EF/R7 protections remain active. This is source/automated activation evidence, not retained-database or operator acceptance.
...
Normal activated whole-root correction safely blocks any current Reversed line. The alpha.9 Restore/RemainReversed dialog now collects an explicit disposition for every reversed permanent line, applies all Restore choices through one separately sequenced atomic logical mutation, re-previews authoritative state and continues batch correction only when the fresh root is all-Active. Batch #30 remains protected and unresolved for operator acceptance. Native Audit/History detail and this UI are source/focused-test complete; closure still requires broader activated-system gates, retained-database rehearsal and Windows acceptance.

Current release: **v0.5.0-alpha.9.4**
...
The alpha.9.3 canonical BAT passed 800/800 automated tests with zero failures/skips and 0 build warnings/errors. Renewed disposable Windows smoke proved the primary FIX4 output-batch route, then exposed that entering whole-root correction through the older original physical Batch #1 still populated the dialog from superseded physical rows (#1/#2, 17/09) while the fresh logical preview correctly identified current rows (#6/#8, 16/09). Execution remained rooted in the fresh logical generation/current line IDs, so no stale ledger authority was created, but the stale operator baseline is acceptance-blocking. BT-20-FIX5/alpha.9.4 makes the dialog projection come from the validated current logical preview; the selected physical batch remains anchor evidence only.

The alpha.9.1 Windows P4 smoke reached the Resolve Reversed Batch Lines dialog and proved first-run startup, synthetic batch/reversal setup, interception, no default decision, Cancel/Esc no-write behavior and unclipped presentation at the tested scaling. It also exposed that the dialog made raw lineage and master-data IDs primary instead of explaining the customer, container and movement. BT-20-FIX3 corrects that presentation contract in alpha.9.2; the complete corrected dialog and disposition/continuation paths still require renewed Windows smoke. The retained database has not been migrated or rehearsed and Batch #30 remains untouched.

The alpha.9.2 canonical gate passed 795/795 automated tests with zero failures/skips and the expanded disposable Windows smoke proved the end-to-end Restore/RemainReversed and subsequent correction lineage listed in Testing. It then exposed that `Correct Entire Batch` rejected a valid lineage-native physical output because presentation compared that evidence batch with immutable original `RootMovementBatchId`. It also showed restoration as `Reversal` in Movement History and benefited from a larger disposition-dialog default. BT-20-FIX4 corrects these items in alpha.9.3 at source/automated level; renewed Windows smoke is still required. The retained database, protected Batch #30 and preserved smoke database were not accessed or mutated during FIX4.
...
The Daily Print Pack cross-section snapshot gap is active in normal schema17 runtime: both delegated numerical sections and their customer/container metadata share one read transaction. Deterministic concurrency evidence remains green. Real-print and Windows/operator acceptance are not implied.

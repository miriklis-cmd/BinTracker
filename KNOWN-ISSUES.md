...
Task20C characterized activation blockers: normal initialization accepts schema17/future schemas without activation health checks; Single used raw sums and recomputed retry balance after later activity; enabled schema17 composition left legacy correction/reversal commands callable; native audit root IDs can collide with unrelated physical movement IDs; review bypasses native operation/audit health; and EF SetNull can nullify tracked immutable membership even though schema17 constraints roll back attempted persisted batch deletion. These defects are not accepted behavior. Task20D fixes startup rejection/readiness through the production R4/R5 coordinator in explicit isolated composition. Task20E fixes the R1 Single behavior under explicit schema17 composition with typed receipts, exact replay and atomic rollback. Task20F fixes R2 there: stable logical previews capture current state/generation, stale execution loses without refresh, every enabled17 alpha.8 mutation route rejects before artifacts, and a Reversed line blocks both current whole-root preview and direct execution even when the command embeds Restore/RemainReversed decisions. Normal schema16 compatibility still uses the accepted legacy paths. Normal initialization remains unchanged and must join the coordinator only at atomic runtime cutover after R3 closure. The remaining retained future failures are R3/EF membership/R7.

R7 deterministic interleavings prove separate snapshot gaps in Dashboard attention threshold, customer active/name plus position, container activity/name/order plus visible position, and Import comparison's retained previous-run effects versus current projection. Daily Print Pack's shared snapshot remains frozen and is not reopened. Task20D coordinator, Task20E receipt and Task20F R2 evidence plus the remaining audit/EF/R7 blockers are recorded in Testing/CONTINUATION; full-suite/BAT runs are temporarily suspended while red tests remain.
